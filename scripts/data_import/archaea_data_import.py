...
def mag_archaea_import():
    ...
    for chunk in pd.read_csv(archaea_file_path, sep='\t', chunksize=BATCH_SIZE):
        ...
        for _, row in chunk.iterrows():
            obj = MAGArchaea(
                archaea_id_GCA=next((id_ for id_ in row['Archaea_ID'].split(', ') if id_.startswith('GCA_')), None),
                archaea_id_GCF=next((id_ for id_ in row['Archaea_ID'].split(', ') if id_.startswith('GCF_')), None),
...
def mag_archaea_protein_import():
    ...
    for chunk in pd.read_csv(archaea_protein_file_path, sep='\t', chunksize=BATCH_SIZE):
        ...
        print(f'{archaea_protein_file_path} Archaea Protein records have been imported successfully')
...
def unmag_archaea_protein_import():
    ...
    for chunk in pd.read_csv(archaea_protein_file_path, sep='\t', chunksize=BATCH_SIZE):
        ...
        archaea_protein_created_num += len(objs)
        print(f'{archaea_protein_file_path} unMAG Archaea Protein records have been imported successfully')
