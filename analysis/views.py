...
def submit_task(request):
    ...
    usertask = str(int(time.time())) + '_' + str(random.randint(1000, 9999))
    uploadfilepath = settings.USERTASKPATH + '/' + usertask + '/upload/'
    ...
    with open(path, 'r') as file:
        ...
        if is_upload:
            ...
            newtask = Task.objects.create(
                ...
                microbial_type=request.data['microbialtype'], analysis_type=request.data['analysistype'],
                modulelist=modulelist, status='Created'
            )
    ...
    with open(path, 'r') as file:
        ...
        if is_upload:
            ...
            if is_multi:
                ...
                newtask = Task.objects.create(
                    name=name, user=request.data['userid'], uploadpath=usertask,
                    microbial_type=request.data['microbialtype'],
...
def view_task_result(request):
    ...
    path = settings.USERTASKPATH + '/' + task.uploadpath + '/output/result/phage.tsv'
...
def view_task_result_proteins(request):
    ...
    taskid = request.query_params.dict()['taskid']
    task = Task.objects.get(id=taskid)
    path = settings.USERTASKPATH + '/' + task.uploadpath + '/output/result/protein.tsv'
    originpath = (settings.USERTASKPATH + '/' + task.uploadpath +
                  '/output/rawdata/annotation/emapper_out.emapper.annotations')
...
def view_task_result_plasmid_fasta(request):
    ...
    phageid = request.query_params.dict()['phageid']
    task = Task.objects.get(id=taskid)
    path = settings.USERTASKPATH + '/' + task.uploadpath + '/output/result/' + phageid + '/sequence.fasta'
    file = open(path, 'rb')
...
def view_task_result_modules(request):
    ...
    task = Task.objects.get(id=taskid)
    path = settings.USERTASKPATH + '/' + task.uploadpath
...
def view_task_result_tree(request):
    ...
    if 'clsuter_id' in query_params:
        ...
        file_path = (settings.USERTASKPATH + '/' + task.uploadpath + '/output/rawdata/tree/' + clsuter_id +
                     '/sequence.phy')
    else:
...
def view_task_result_arvgs(request):
    ...
    arvg_argpath = taskpath + ('/output/rawdata/arvf/antimicrobial_resistance_gene_result'
...
def view_task_trnas(request):
    ...
    phageid = request.query_params.dict()['phageid']
    task = Task.objects.get(id=taskid)
    path = settings.USERTASKPATH + '/' + task.uploadpath + '/output/result/trna.tsv'
