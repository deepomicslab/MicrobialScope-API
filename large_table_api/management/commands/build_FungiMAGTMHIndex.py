...
from large_table_api.models import FungiMAGTMHIndex
import pandas as pd
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 清空现有索引
        FungiMAGTMHIndex.objects.all().delete()
        ...
        for tsv_file in tqdm(tsv_files):
            ...
            # 添加到批量列表
            index_objects.append(FungiMAGTMHIndex(
            ...
            if len(index_objects) >= batch_size:
                FungiMAGTMHIndex.objects.bulk_create(index_objects)
                index_objects = []
